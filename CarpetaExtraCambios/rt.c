		...
		case 2:
			{
				...
				esfera.Ka=data[5];
				esfera.Kn=data[6];
				esfera.normalVector = sphereIntersection;
	...
	*/
	...
	if (file = fopen(escenaFile, "r")){
		...
		while (fgets(temporalBuffer, 100, file)!=NULL){//Mientras el archivo siga teniendo algo
			...
			if (strstr(temporalBuffer, "Luz")!=NULL){
				numberLights++;
				continue;
			}else if (strstr(temporalBuffer,"Esfera")!=NULL){
				numberObjects++;
				printf("%s", temporalBuffer);
				continue;
			}else if (strstr(temporalBuffer, "Poligono")!=NULL){
				numberObjects++;
				printf("%s", temporalBuffer);
				continue;
			}else if (strstr(temporalBuffer, "Cono")!=NULL){
				numberObjects++;
				printf("%s", temporalBuffer);
				continue;
			}else if (strstr(temporalBuffer, "Cilindro")!=NULL){
				numberObjects++;
				printf("%s", temporalBuffer);
				continue;
			}
