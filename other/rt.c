...
struct Color getColor(struct Vector anchor, struct Vector direction, int rLevel){
	...
	if(intersection.null == 1){
		...
		if(rLevel > 0){
			...
			color = ponderColor(color, reflectionColor, 0.2, 0.2);
		}else{
			color = ponderColor(color, background, 0.2, 0.2);
		}
		...
		color = ponderColor(color, transparencyColor, 1, 0.6);
	}
	return (color);
}
...
void createObjectFromData(long double *data, int whichObjectCreate, int quantityData, struct PlaneCut* planeCutsFound, struct Texture* texturesFound, struct DraftPlane *draftPlanesFound, int numberPlaneCuts, int numberTextures, int numberDraftPlanes){
    ...
    switch(whichObjectCreate){
        ...
        case 3: { //Polígonos
            ...
            if (debug == 1){
                int inicioPlano = 10+numVertexesPolygon;
                ...
                printf("Esquina superior derecha (%LF, %LF, %LF) \n", data[inicioPlano+6],data[inicioPlano+7],data[8]);
            ...
            for (int i =0; i+7 < quantityData-12;){  //Los 12 del plano se ignoran
            ...
            for (int i =0; i+7 < quantityData-12;){ //Los 12 son los 4 vértices del plano
                vertex.x = data[10+i];
                i++;
                vertex.y = data[10+i];
                i++;
...
struct Color **getTexels(char* pFile){
    int counter, x, y, i, j;
    char dump[100];
    time_t t;
    struct Color **temp;
    ...
    if (file = fopen(pFile, "r")) {
        ...
        char temporalBuffer[30];
        int i = 0,x = 0, y = 0;
        while (fgets(temporalBuffer, 30, file)!=NULL){ //Mientras el archivo siga teniendo algo
            ...
            //printf("%LF \n", number);
            if (i == 0){
                temp[hRes - x -1][y].r = (number)/255;
            }else if(i == 1){
                temp[hRes - x -1][y].g = (number)/255;
            }else if(i == 2){
                temp[hRes - x -1][y].b = (number)/255;
            }
            i = (i+1)%3;
            if (i==0){
                y = (y+1)%vRes;
            ...
            //printf("Color leído: (%LF, %LF, %LF) \n",temp[hRes - x -1][y].r*255, temp[hRes - x -1][y].g*255, temp[hRes - x -1][y].b*255 );
        ...
        //printf("%s", temporalBuffer);
            ...
            if (strstr(temporalBuffer, "NumberTextures")!=NULL || strstr(temporalBuffer, "NumberTexturas")!=NULL){
                ...
                *posAfterReading = ftell(file);
                ...
                struct Color **textureMap=getTexels(texturesFound[indexTexture].filename);
